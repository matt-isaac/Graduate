...
bayes.er <- data.frame(n = c(100, 200, 500, 1000), er = double(4))
logistic.er <- data.frame(n = c(100, 200, 500, 1000), er = double(4))
...
knn <- train(as.factor(y) ~ x, data = ds100, method = 'knn', trControl = trainControl(method = 'cv', number = 5))
...
for(d in ds){
  data <- d
  preds <- double(nrow(data))
  for(i in 1:nrow(data)){
    if(data$x[i] >= b.cutoff){
      pred.class <- 1
    } else {
      pred.class <- 0
    }
  ...
  data$preds <- preds
  data$misclass <- double(nrow(data))

  for(i in 1:nrow(data)){
    if(data$preds[i] == data$y[i]){
      data$misclass[i] <- 0
    } else {
      data$misclass[i] <- 1
    }
  ...
  bc.mr <- sum(data$misclass)/nrow(data)
  bayes.er$er[ncount] <- bc.mr
  ncount <- ncount + 1
}
